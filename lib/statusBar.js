...
class StatusBar {
  ...
  /** Apply ANSI scroll region with gap above status bar */
  _applyScrollRegion() {
    const rows = process.stdout.rows || 24;
    const gap = 3; // blank lines between content and status bar
    const scrollEnd = rows - 1 - gap;
    // Set scroll region to exclude bottom row + gap
    process.stdout.write(`\x1b[1;${scrollEnd}r`);
    // Clear the gap rows so no stale text remains
    for (let r = scrollEnd + 1; r < rows; r++) {
