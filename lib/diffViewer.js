...
function formatOperationsBatch(operations, fileManager) {
  ...
  // Header
  output.push(`\n${colors.orange}${'═'.repeat(60)}${colors.reset}`);
  output.push(`${colors.orange}  PROPOSED CHANGES (${operations.length} files)${colors.reset}`);
  output.push(`${colors.orange}${'═'.repeat(60)}${colors.reset}\n`);
