...
class InlineComplete {
   ...
   * DISABLED: Ghost text ANSI escape sequences (CURSOR_BACK, ERASE_TO_EOL)
   * race with readline's internal cursor state, causing the cursor to appear
   * ahead of typing and breaking backspace. Ghost suggestion is still tracked
   * internally so Tab/Right acceptance works, but nothing is visually rendered.
   *
  ...
  renderGhost(ghost) {
    ...
    // Visual rendering disabled to fix cursor/backspace issues.
    // The ghost value is still stored so accept() / hasGhost() work for Tab.
  }

  /**
   * Clear any currently displayed ghost text.
   * Since visual rendering is disabled, just reset the internal state.
   */
  clearGhost() {
    if (!this.currentGhost) return;
    // No ANSI cleanup needed since we no longer render ghost visuals.
    this.currentGhost = '';
  }
