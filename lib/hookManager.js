 ...
 *   - Global:  ~/.ripley/hooks.json   (default, applies to all projects)
 *   - Project: <projectDir>/.ripley/hooks.json  (overrides global by name)
 *
...
const PROMPT_HOOK_SYSTEM = `You are an expert at configuring automated hooks for an AI coding assistant called Ripley Code.
...
class HookManager {
  constructor(options = {}) {
    ...
    this.globalDir = path.join(os.homedir(), '.ripley');
  ...
  _projectHooksPath() {
    return path.join(this.projectDir, '.ripley', 'hooks.json');
  }
  ...
  async runAddWizard(pickFn, askQuestion, modelRegistry) {
    // 1. Scope
    const scopeItems = [
      { key: 'global', label: 'Global', description: 'Applies to all projects (~/.ripley/hooks.json)' },
      { key: 'project', label: 'Project', description: 'Only this project (.ripley/hooks.json)' }
    ];
