...
class SubAgentManager {
  ...
  async spawn(modelKey, task, options = {}) {
    ...
    // Resolve model
    const resolved = this.modelRegistry.resolveModelKey(modelKey);
    if (!resolved) {
      return {
        error: `Could not resolve model "${modelKey}". Use /agent models to see available models.`,
        tokens: 0,
        toolCalls: 0
      };
    }
...
const SPAWN_AGENT_TOOL_DEF = {
  ...
  function: {
    name: 'spawn_agent',
    description: 'Spawn a sub-agent on a different model. Use for code review, parallel research, complex reasoning, or delegating to specialized models.',
    parameters: {
      type: 'object',
      properties: {
        model: {
          type: 'string',
          description: 'Model in "provider:alias" format. e.g., "anthropic:claude-sonnet-4.6", "openai:codex-5.3-high", "local:current"'
        },
        task: {
          type: 'string',
