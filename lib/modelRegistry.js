...
class ModelRegistry {
  ...
  /**
   * Get the preferred prompt name for the current model.
   * Falls back to 'code-agent' if not specified.
   */
  getPrompt() {
    const name = this.getCurrent();
  ...
  /**
   * Get inference settings for the current model.
   * Returns { temperature, topP, repeatPenalty } or defaults.
   */
  ...
  /**
   * Check if current model requires reasoning field preservation (GPT-OSS).
   */
